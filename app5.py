...
try:
    ...
    with open("tokenizer2.json", "r") as f:
        tok_data = json.load(f)
        tokenizer = tokenizer_from_json(json.dumps(tok_data))
    label_classes = np.load("label_classes.npy", allow_pickle=True)
    with open("scaler.pkl","rb") as f:
        scaler = pickle.load(f)
    st.success("CNN and scaler artifacts loaded successfully!")
...
# -------------------------
# Task 1: Text Classification & Recommendations
# -------------------------
...
if st.button("Classify & Recommend"):
    if cnn_model and tokenizer and label_classes is not None:
...
try:
    ...
    pass  # fallback to Isolation Forest
